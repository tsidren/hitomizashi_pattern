...
def color_m(grid, first_color, second_color, bina, bina2):
    color1 = first_color
    color2 = second_color
    for i in range(2, len(bina2)):
        if int(bina2[i]):
            color1 = first_color
            color2 = second_color
        else:
    ...
    for j in grid:
        for i in range(2, len(j), 2):
            color_one(j, i, color2)
    for i in range(1, len(bina), 2):
        color_one(bina, i, color1)


b = ar_m(ar_c(binary1), len(binary1))
color_m(np.transpose(b), (214, 166, 190), (162, 195, 200), binary1, binary2)
